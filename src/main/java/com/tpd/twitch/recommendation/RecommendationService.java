package com.tpd.twitch.recommendation;
...
import com.tpd.twitch.favorite.FavoriteService;
...
public class RecommendationService {
    private static final int MAX_GAME_SEED = 3;  // If you write too much, the API response time will be too long.
    ...
    // To implement recommendation, we need 3 private APIs (helper functions) to perform this task:
    // 1. Exclude items that have already been favorited.
    // 2. Twitch provides different APIs - Streams can be received in bulk, but Clips and Videos can only be received one at a time.
    //      So, for Videos, we need to search for each game ID separately in a for loop.
    // 3. Recommended Clips are similar to Videos - they require two for loops.
